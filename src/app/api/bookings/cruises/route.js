...
export async function POST(request) {
  try {
    ...
    const doc = await FlexBooking.create({ ...body, refCode, type: 'cruise', status: 'confirmed' });
...
export async function GET(request) {
  try {
    ...
    if (ref) {
      const booking = await FlexBooking.findOne({ $or: [{ refCode: ref }, { bookingRef: ref }], type: 'cruise' }).lean();
      return NextResponse.json({ booking });
    }
    const bookings = await FlexBooking.find({ type: 'cruise' }).sort({ createdAt: -1 }).limit(50).lean();
