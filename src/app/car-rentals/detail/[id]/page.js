...
function CarDetailContent() {
  ...
  const bookUrl = `/car-rentals/booking/${params.id}?price=${price}&name=${encodeURIComponent(name)}&company=${encodeURIComponent(company)}&type=${type}&days=${days}`;
  ...
  return (
    <div style={{ paddingTop: '68px' }}>
      <div className="hd-nav"><div className="hd-nav-inner">
        <div className="hd-crumbs"><Link href="/car-rentals/"><span>Home</span></Link><span className="hd-crumbs-sep">›</span><Link href="/car-rentals/"><span>Car Rentals</span></Link><span className="hd-crumbs-sep">›</span><Link href="/car-rentals/results"><span>Results</span></Link><span className="hd-crumbs-sep">›</span><span className="hd-crumbs-active">{name}</span></div>
        <div className="hd-nav-right"><Link href="/car-rentals/results" className="hd-nav-btn"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6" /></svg> Back</Link><button className="hd-nav-cta" onClick={() => showLoading('car-booking', () => router.push(bookUrl))}>Reserve · ${price}/day</button></div>
      </div></div>
        ...
        <div>
          ...
          <div className="hd-sec"><div className="hd-sec-title">Renter reviews</div>
            ...
            <div className="hd-rev-grid">
              {[{ ini: 'TJ', n: 'Tyler J.', dt: 'Aug 2025 · Denver', txt: 'Rented through Fareoworld — $30 cheaper per day than direct. Car was spotless.' },
